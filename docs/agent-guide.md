...
| `guest_id` | int | — | force a specific detected identity as the guest (see 3.4) |
...
| `GET /api/jobs/{id}/report.json` | per shot: timestamp `t` (seconds), `score`, `solo`, and `parts.llm_note` (one-sentence reason), `parts.llm_eyes` (`open`) |
